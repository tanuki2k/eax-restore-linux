...
if [ "$EUID" -eq 0 ]; then
    ...
    echo -e "\n${WHITE}Running Wine or Protontricks as root will permanently break your prefix permissions.${NC}"
...
if [ -n "$SteamEnv" ] || [ -n "$STEAM_COMPAT_DATA_PATH" ]; then
    ...
    echo -e "\n${WHITE}This script requires keyboard input and terminal interaction.${NC}"
