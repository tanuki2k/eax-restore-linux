...
**Contributing to the known games database:** PRs adding or correcting entries in `known-eax-games.json` are welcome. Please only add a store `id` you've independently verified against the storefront's own page or API — a wrong ID would point the script at someone else's prefix. The file starts with `"schema_version": 2`; `games` is an array of entries with this shape (see the existing entries):
...
- `stores` — an object with a `"steam"` and/or `"gog"` key; **omit a store key entirely when the game isn't sold there** (a GOG-only game has no `"steam"` key, so "GOG only" needs no prose). Each store block holds:
  ...
  - `listing` — `"available"` or `"delisted"` (pulled from sale; existing owners keep access). Availability only, not pricing.
  - `api` — `null` to inherit `default_api`, or `"directsound3d"` / `"openal"` to override it for **this store's build only**. Set it only where a storefront's current build genuinely differs (e.g. Thief Gold / Thief II: `default_api` is `"directsound3d"`, but GOG's build comes with NewDark pre-installed, which runs audio and EAX reverb through OpenAL, so its `gog` block has `"api": "openal"`).
  - `id_confidence` — `"storefront_verified"` (the default — you checked the ID against the store's own page/API), `"local_install_verified"` (also confirmed against a real local install), or `"steamdb_historical"` (the store page is gone; the ID is SteamDB's historical record, not checked locally). Only `"steamdb_historical"` shows a line in the game-details display.
  - `store_details` — free-text prose about what's specific to **this store's listing**: engine/audio-path differences ("It comes with NewDark pre-installed…"), delisting circumstances, bundling/packaging quirks ("Heroic installs this bundle as two separate library entries"), and build-identity/disambiguation facts ("Both stores sell this as the 'Planetary Pack'", "GOG sells this bundled with the BFG Edition remaster", "This is the pre-2017 classic build, not the Enhanced Edition"). `null` when there's nothing store-specific to say. When a fact applies identically to more than one store (e.g. both stores sell the same bundle), duplicate the prose into each relevant store block rather than only one — and write each store's copy to stand on its own, never referencing "the other store's block" or "the Steam version" from within the GOG block (a user viewing one store's game-details only ever sees that one block, not both side by side). Don't name the script's own parts (DSOAL / OpenAL Soft) or talk about "paths" / "intercepting" — state the fact and let the script say what it will use.
  - `patches` — free-text prose for **this store's build**, shown under "Suggested community patches:": community patch/mod recommendations (TFix / T2Fix, SilentPatch, Sneaky Upgrade, SCP…), including ones that — for a delisted game — also happen to be the practical way to get a legitimate full copy (e.g. OldUnreal's patches, which both fix the game and are its current distribution channel; frame a community OpenAL reimplementation as a strong alternative worth using instead of this script's approach, not as an inferior fallback). `null` if there's nothing store-specific to recommend. (Not for "go buy it from a different retailer" content that has nothing to do with a patch/mod project — that stays in `notes` if it ever comes up.)
  - `beta_branch` — Steam only. The exact `BetaKey` name (right-click the game -> Properties -> Betas) of a beta branch that restores EAX on this store's build — set only when `restore_details` names one. `null`/omit otherwise. Lets the install-time check offer to detect whether the user's local appmanifest already has this branch opted into/mounted, instead of always showing the `removed_by_patch` warning.
  - `exe_path` — the main game `.exe`'s path relative to the install folder, with forward slashes and exact case, e.g. `"SCDA-Offline/System/SplinterCell4.exe"` for Splinter Cell: Double Agent (which installs a root launcher plus separate single-player and multiplayer builds). `null`/omit otherwise. Set it only for a title whose layout makes the script's exe auto-detection pick the wrong folder, and only after checking the path against a real install. When the file exists, its folder is offered first in the "Locate Game Executable" step; when it doesn't, detection carries on as normal.
- `default_api` — `"directsound3d"` or `"openal"`. The game-level baseline audio API, used for any store without its own `api` override and for non-store installs. Only meaningful when `eax_status` is `"supported"`; drives the Audio API Detection step's remediation choice.
- `recommended_tweaks` — array of strings from `"eax_unified"`, `"expand_audio_limits"`, `"com_registry_routing"`. **Omit the key entirely (or use `[]`) when nothing is recommended.** Each drives a default-Y prompt in the Advanced Compatibility Tweaks step *before* that step's generic opt-in gate, instead of the generic default-N prompt:
  - `"eax_unified"` — for the ~30 titles that reach EAX through Creative's `eax.dll` "EAX Unified" shim rather than native DirectSound3D. Drives a guarded prompt for the EAX Unified Dummy Files tweak (it checks whether the game ships its own `eax.dll` before offering the dummy-file tweak).
  - `"expand_audio_limits"` — for a title independently verified to need the Expand Audio Limits tweak specifically, e.g. F.E.A.R.'s audio dropping out during large firefights.
  - `"com_registry_routing"` — for a title independently verified to need the COM Registry Routing tweak specifically, e.g. Grand Theft Auto: San Andreas.
...
- `build_workaround_available` — `true`/omit; only meaningful when `eax_status` is `"removed_by_patch"`. Set to `true` only when `restore_details` documents an in-place fix within the *same store install* the script is already pointed at (e.g. a Steam beta branch) — in that case the install-time check warns and still offers "Continue installing anyway?". Omit it (the default) when the only fix is an entirely separate install the script wouldn't be scanning this run (e.g. a pre-patch retail/CD copy) — then the check hard-blocks with no continue-anyway bypass, the same as `"not_implemented"`.
- `eax_status_details` — free-text prose explaining *why* the current build is in the state `eax_status` describes (the problem), shown in the game-details display's `Status:` block. `null` when `eax_status` is `"supported"` and there's nothing worth explaining. Since every entry here is a current digital-store listing, don't add "make sure you're patched to at least version X" trivia — the storefront build is definitionally current; only note a version gate if you've independently confirmed a store's build can still ship pre-patch.
- `restore_details` — free-text prose pointing at how to make EAX actually work (the fix), shown as its own `The fix:` block: how to get a build where EAX still works (a Steam beta branch, a retail/CD copy…), in-game or config-file activation steps (menu toggles, `.ini` settings), and Advanced Compatibility Tweak cross-references (e.g. the Expand Audio Limits or COM Registry Routing tweaks). `null` if none needed. `eax_status_details` is the "why", this is the "how". Not per-store — it's the game-level fix regardless of which store it was bought from.
- `notes` — free-text prose caveats that aren't covered by the fields above: cross-references to sibling entries, limitation/expectation-setting caveats ("reverb here is subtle, that's expected not a bug"), controller/multiplayer/mod quirks. `null` when there's nothing to say.
- `eax_versions` — array of supported EAX version strings; only set from a verifiable source (the game's manual/readme, an in-game audio settings menu, or a maintained compatibility database) — cite it in the PR description. `[]` for a `"not_implemented"` entry.
