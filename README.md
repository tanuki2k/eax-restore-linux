...
- `steam_api` / `gog_api` — optional, nullable per-store overrides for `api`, used only when a specific store's current build genuinely differs from the other (e.g. Thief Gold: Steam's build still speaks DirectSound3D, but GOG's ships pre-patched with NewDark's OpenAL Soft support). Omit entirely when both stores share the same `api`.
