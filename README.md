...
- `stores` — an object with a `"steam"` and/or `"gog"` key; **omit a store key entirely when the game isn't sold there** (a GOG-only game has no `"steam"` key, so "GOG only" needs no prose). Each store block holds:
  ...
  - `build_notes` — free-text prose about what's specific to **this store's build**: "It comes with NewDark pre-installed…", "GOG sells this as the 'Fortune's Edition' bundle", "This is the pre-2017 classic build, not the Enhanced Edition", "Heroic installs this bundle as two separate library entries". `null` when there's nothing store-specific to say. Don't name the script's own parts (DSOAL / OpenAL Soft) or talk about "paths" / "intercepting" — state the build fact and let the script say what it will use.
  - `patches` — free-text prose recommending community patches for **this store's build** (TFix / T2Fix, SilentPatch, Sneaky Upgrade, SCP…), shown under "Suggested community patches:". `null` if none.
...
- `eax_status_notes` — free-text prose explaining *why* the current build is in the state `eax_status` describes (the problem), shown in the game-details display's `Status:` block. `null` when `eax_status` is `"supported"` and there's nothing worth explaining.
- `eax_restore_hint` — free-text prose pointing at how to get a build where EAX still works (the fix — a Steam beta branch, a retail/CD copy…), shown as its own `The fix:` block. `null` if none exists. `eax_status_notes` is the "why", this is the "how".
- `notes` — free-text prose caveats that aren't tied to one store: in-game menu steps to turn EAX on, renderer-choice caveats for user-added installers, "reverb here is subtle, that's expected not a bug", controller/multiplayer/mod quirks, cross-references to sibling entries. `null` when there's nothing to say.
