...
- `eax_unified` — `true` only for the ~30 titles that reach EAX through Creative's `eax.dll` "EAX Unified" shim rather than native DirectSound3D. **Omit the key entirely otherwise.** Drives a guarded prompt in the Advanced Compatibility Tweaks step (it checks whether the game ships its own `eax.dll` before offering the dummy-file tweak).
