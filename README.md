...
- `default_api` — `"directsound3d"` or `"openal"`. The entry's baseline audio API: used for physical-media installs and for any storefront without its own override. Only meaningful when `eax_status` is `"supported"`; drives the Audio API Detection step's remediation choice.
- `steam_api` / `gog_api` — optional, nullable per-store overrides for `default_api`, set only on the store whose current build genuinely differs from the baseline (e.g. Thief Gold / Thief II: `default_api` is `"directsound3d"`, but GOG ships NewDark pre-applied, which routes audio and EAX reverb through OpenAL, so `gog_api` is `"openal"`). Omit both when every store matches `default_api`.
